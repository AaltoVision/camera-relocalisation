...
nbatches = math.floor(training_data_size_ / opt.train_batch_size)
...
function train()
    ...
    --epoch = epoch or 1
    train_err_orientation_ = 0
    train_err_translation_ = 0
    
    print(c.blue '==>'.." online epoch # " .. epoch_ .. ' [batchSize = ' .. opt.train_batch_size .. ']')
    
    local time_rnd_training = sys.clock()
    local indices = torch.randperm(training_data_size_):long():split(opt.train_batch_size)
    indices[#indices] = nil
    ...
    for t,v in ipairs(indices) do
        ...
        --optim.sgd(feval, parameters, optim_state_sgd)
        cutorch.synchronize()
    end
    cutorch.synchronize()
    collectgarbage()
    ...
    time = sys.clock() - time
    print(c.blue '==>' .. " time taken for 1 epoch = " .. (time) .. " s, time taken to learn 1 sample = " .. ((time/training_data_size_)*1000) .. ' ms')
    print(c.blue '==>' .. " learning rate: " .. optimState.learningRate)
    print(c.blue '==>' .. " scene: " .. opt.scene)
